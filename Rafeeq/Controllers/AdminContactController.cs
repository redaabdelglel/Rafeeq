...
namespace Rafeeq.Controllers.Admin
{
    ...
    public class AdminContactController : ControllerBase
    {
        ...
        {
            var result = await _contactService.GetMessagesCountAsync();
            if (!result.success)
                return BadRequest(new { success = false, message = result.Message });

            return Ok(new { success = true, result.count });
        }

        [HttpPost("reply")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> ReplyToMessage([FromBody] CreateReplyDto dto)
        {
            var responderId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
            var result = await _contactService.AddReplyAsync(dto, responderId);
            if (!result.Item1) return BadRequest(new { success = false, message = result.Item2 });
